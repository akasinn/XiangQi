...
struct move_t{
    ...
    void calc_ds(const string &board){
        ...
        if(is_red(board[now]))ds=-ds;//動かした側が得になる動きを正とする
        return;
    }
};
...
move_t consider_all_options(const string &board,const int a){
    const vector<move_t> Move=f_all_options(board);
    move_t ans={-1,-1,-100};
    if(a<1){
        for(move_t move:Move){
            if(ans.ds<move.ds)ans=move;
        }
        return ans;
    }
    for(move_t move:Move){
        const string board_=move_a_piece(move,board);
        const move_t move_=consider_all_options(board_,a-1);
        move.ds-=move_.ds;
        if(ans.ds<move.ds)ans=move;
    }
    return ans;
...
move_t solve(string board){
    return consider_all_options(board,4);
